...
;(function($){
    ...
    $.fn.josa = function(josaInput){ // "이/가", "을/를", "은/는", "와/과", "로/으로" 중 하나를 인자로 받아야 한다.
        ...
        var josaOutput = {
            "josa0"       : ["이(가)", "을(를)", "은(는)", "와(과)", "로(으로)"], // 어떤 조사를 붙여야 할지 알 수 없는 경우 이 값 중 하나를 출력.
            "josa1"    : ["이", "을", "은", "과", "으로"], // 종성 있는 경우 이 값 중 하나를 출력.
            "josa2"   : ["가", "를", "는", "와", "로"] // 종성 없는 경우 이 값 중 하나를 출력.
        };
        ...
        $(this).each(function(){
            ...
            var printJosa = function(){
                ...
                if( !/([가-힣]|\d|[a-z])$/i.test(str) ){ // 어떤 조사를 붙여야 할지 모르는 경우.
                    setJosa(josaOutput.josa0); // ["이(가)", "을(를)", "은(는)", "와(과)", "로(으로)"] 를 인자로 넘긴다.
                } else if( hasFinal ){ // 발음에 종성 있는 경우.
                    setJosa(josaOutput.josa1); // ["이", "을", "은", "과", "으로"] 를 인자로 넘긴다.
                } else if( !hasFinal ){ // 발음에 종성 없는 경우.
                    setJosa(josaOutput.josa2); // ["가", "를", "는", "와", "로"] 를 인자로 넘긴다.
                }
